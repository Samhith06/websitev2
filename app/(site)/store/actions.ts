...
export type RedeemOutcome =
  | { ok: true; message: string; balance: number; pendingReview: boolean }
 ...
 * off to `lib/store/shop.ts`, where the debit, the stock decrement and the
 * redemption row are one transaction. Everything that could refuse — stock,
 * cooldown, balance — is checked there, against locked rows, rather than here
 * where it would race.
 */
export async function redeemItem(itemId: number): Promise<RedeemOutcome> {
  ...
  if (gate.user.status === 'frozen') {
    return { ok: false, error: 'This account is frozen, so the store is closed for now.' };
  }
  ...
  return {
    ...
    pendingReview: result.needsReview,
