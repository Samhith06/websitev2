...
export default async function AdminOverview() {
  ...
  const [feed, db, flow, rounds, earning, members, tickedAt, auditLog] =
    await Promise.all([
      weekly
  ...
  const ticking = Boolean(
    tickedAt && Date.now() - new Date(tickedAt).getTime() < 10 * 60_000,
  );
  ...
  return (
    <>
      ...
      <Hairlines cols="grid-cols-2 lg:grid-cols-4">
        ...
        />
        <Stat
          label="Earning right now"
          value={maybe(earning)}
          sub={`${maybe(members)} accounts`}
        />
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        {/* --------------------------------------------------------- */}
        {/* Stream Control (Manual)                                   */}
        {/* --------------------------------------------------------- */}
        <StreamControl initialLive={stream.live} />
          ...
          <div className="p-4">
            ...
            {pending.length === 0 ? (
              <p className="mt-4 text-[13px] leading-relaxed text-muted">
                Nothing waiting. Entries and Discord roles are granted the
                moment they are bought and never reach this queue.
              </p>
            ) : (
                ...
                {pending.map((r) => (
                  ...
                  >
                    <span className="min-w-0 truncate text-ink-2">
                      {r.itemName}{" "}
                      <span className="text-faint">· {r.member}</span>
                    </span>
        ...
        <div className="grid gap-px bg-line sm:grid-cols-3 [&>div]:bg-surface">
          ...
          <div className="px-4 py-4">
            ...
            <Num className="text-[24px]">
              {minted !== null && destroyed !== null
                ? `+${coins(minted - destroyed)}`
                : "—"}
            </Num>
          </div>
        </div>
        <p className="border-t border-line px-4 py-3 text-[12.5px] leading-relaxed text-muted">
          At 99% RTP the games are close to neutral, not a real sink. If coins
          inflate, pull the daily wager cap or nudge shop prices — never cut the
          advertised RTP.
        </p>
      </Card>
      ...
      <Card className="mt-4">
        <div className="flex items-center justify-between gap-3 border-b border-line px-4 py-3">
          ...
          <Link
            href="/admin/audit"
            className="font-mono text-[11px] uppercase tracking-[0.14em] text-brand hover:text-brand-dim"
          >
            Full log
          </Link>
        </div>
        <ul className="divide-y divide-line">
          {auditLog.length === 0 ? (
            <li className="px-4 py-6 text-[13.5px] text-muted">
              Nothing logged yet. Admin actions that change something are
              recorded here.
            </li>
          ) : null}
          {auditLog.map((entry) => (
      ...
      <p className="mt-6 max-w-2xl text-[12.5px] leading-relaxed text-muted">
        Paid out to date: {maybe(paidOut, money)} across every finalised period.
        Prize records live under Prizes and periods.
      </p>
    </>
  );
}
