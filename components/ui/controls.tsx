...
const variants: Record<Variant, string> = {
  ...
  primary: 'bg-brand text-brand-ink border border-brand hover:bg-brand-dim hover:border-brand-dim',
  outline: 'bg-transparent text-ink border border-line-2 hover:border-brand hover:text-brand-dim',
  ghost: 'bg-transparent text-ink-2 border border-transparent hover:text-ink hover:bg-surface-2',
  danger: 'bg-transparent text-danger border border-danger/45 hover:bg-danger/10 hover:border-danger',
...
function buttonClass({ variant = 'primary', size = 'md', full, className }: ButtonBaseProps) {
  return cn(
...
export function Button({
  ...
  return (
    <button className={buttonClass({ variant, size, full, className, children })} {...rest}>
      {children}
    </button>
  );
}
...
export function ButtonLink({
  ...
  if (external) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer nofollow sponsored" className={cls} {...rest}>
        {children}
      </a>
    );
  }
...
export function Chip({
  ...
  return (
    <button type="button" role="tab" aria-selected={active} className={cls} {...(rest as object)}>
      {children}
    </button>
  );
}
...
export function CountBadge({ children, tone = 'gold' }: { children: React.ReactNode; tone?: 'gold' | 'brand' | 'danger' }) {
  const tones = {
  ...
  return (
    <span className={cn('inline-flex min-w-5 items-center justify-center rounded-full border px-1.5 font-mono text-[10.5px] tabular-nums', tones[tone])}>
      {children}
    </span>
  );
}
...
export function StatusDot({
  ...
  return (
    <span className={cn('inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.14em]', text[tone], className)}>
