 ...
 */
export function ClipCard({ clip, className }: { clip: Clip; className?: string }) {
  return (
    <article className={cn('flex flex-col', className)}>
      <PlayerFrame
        ...
        playSize={48}
      />
      <h3 className="mt-3 text-[15.5px] font-semibold leading-snug text-ink">{clip.title}</h3>
 ...
 */
export function ClipCarousel({ clips, label }: { clips: Clip[]; label: string }) {
  return (
    <div
    ...
    >
      {clips.map((clip) => (
        <ClipCard
          key={clip.id}
          clip={clip}
          className="w-[210px] shrink-0 snap-start lg:w-[300px]"
        />
      ))}
    </div>
  );
