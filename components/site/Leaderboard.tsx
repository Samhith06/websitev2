      ...
      {top.map((row, index) => {
        ...
        return (
          <div key={row.rank} className={cn(step.order, step.lift)}>
            ...
            >
              {first ? (
                <Crown size={15} className="absolute right-3.5 top-3.5 text-gold/70" aria-label="Leader" />
              ) : null}
              ...
              <p className={cn('mt-3 truncate font-semibold text-ink', first ? 'text-[17px]' : 'text-[15px]')}>
                {row.maskedUsername}
              </p>
...
function Movement({ value }: { value: number | null }) {
  ...
  const up = value > 0;
  return (
    <span className={cn('inline-flex items-center gap-1', up ? 'text-brand' : 'text-danger')}>
          ...
          >
            ...
            <Cell className="min-w-0">
              <span className="flex items-center gap-2.5">
                ...
                <span className="min-w-0 truncate text-[14px] text-ink">{row.maskedUsername}</span>
              </span>
 ...
 */
export function ProvenanceRow({ frozen = false, className }: { frozen?: boolean; className?: string }) {
  const [open, setOpen] = useState(false);

  return (
    <div className={cn('overflow-hidden rounded-[12px] border border-brand-line bg-brand-bg', className)}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
